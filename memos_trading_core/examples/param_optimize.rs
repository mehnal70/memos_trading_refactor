...
use memos_trading_core::robot::backtester::{BacktestConfig, BacktestResult, Backtester};
...
/// Sembol başına OOS-skoru en yüksek satırı seçer (semboller alfabetik). Boş → boş.
fn pick_champions(rows: &[OptRow]) -> Vec<ChampionRow> {
    ...
    for r in rows {
        best.entry(r.symbol.as_str())
            .and_modify(|c| { if r.oos_score > c.oos_score { *c = r; } })
            .or_insert(r);
    }
...
fn main() {
    ...
    println!("   (REPORT-ONLY: ParameterStore'a yazmaz. IS-iyi/OOS-kötü = overfit; robust=OOS'ta da tutar)\n");
    ...
    for sym in &symbols {
        for iv in &intervals {
            ...
            optimize_series(sym, iv, &candles, n, is_pct, seed, min_oos_trades, &mut rows);
            scanned += 1;
        }
    }
    ...
    println!("\n══════ SONUÇ ({} seri tarandı · {} atlandı · {} satır · {} ROBUST) ══════",
        scanned, skipped, rows.len(), robust_count);
    if rows.is_empty() {
        println!("  Optimize edilebilir seri yok — filtre çok dar, veri yetersiz/gappy ya da sinyal üretmedi.");
    } else {
        println!("  {:<10} {:<5} {:<14} {:>7} {:>7} {:>7} {:>7} {:>5} {:>4}  en iyi paramlar",
            "symbol", "iv", "strateji", "IS_skor", "OOS_skr", "OOS%", "OOSwin", "işl", "R");
        for r in rows.iter().take(50) {
            let flag = if r.robust { "✅" } else { "—" };
            println!("  {:<10} {:<5} {:<14} {:>7.2} {:>7.2} {:>6.1}% {:>6.0}% {:>5} {:>4}  {}",
                r.symbol, r.interval, r.strategy, r.is_score, r.oos_score,
                r.oos_pnl_pct, r.oos_win_rate, r.oos_trades, flag, r.params_fmt);
        }
    ...
    if !champions.is_empty() {
        let champ_robust = champions.iter().filter(|c| c.robust).count();
        println!("\n══════ ŞAMPİYONLAR (sembol başına kötünün iyisi · {}/{} robust) ══════",
            champ_robust, champions.len());
        println!("  {:<10} {:<5} {:<14} {:>7} {:>7} {:>6} {:>5} {:>4}  en iyi paramlar",
            "symbol", "iv", "strateji", "OOS_skr", "OOS%", "win", "işl", "R");
        for c in &champions {
            let flag = if c.robust { "✅" } else { "—" };
            println!("  {:<10} {:<5} {:<14} {:>7.2} {:>6.1}% {:>5.0}% {:>5} {:>4}  {}",
                c.symbol, c.interval, c.strategy, c.oos_score,
                c.oos_pnl_pct, c.oos_win_rate, c.oos_trades, flag, c.params_fmt);
        }
        println!("  (— = OOS'ta güven kapısını geçmedi; yine de o sembolün en-az-kötü seçeneği. Robust olmayana güvenme.)");
    }
    ...
    println!("\n→ ROBUST (OOS'ta da skor>0 + pnl>0 + işlem≥{min_oos_trades}): {robust_count} / {}",
        report.rows.len());
    println!("  ⚠️  Bu paramları canlıya almadan önce WF çapraz-kontrol + slippage ile teyit et ([[project_edge_scan]] dersi).\n");
}
