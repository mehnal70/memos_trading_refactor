...
fn main() {
    ...
    for sym in &symbols {
        let res = rt.block_on(fetcher.fetch_history_market(sym, &interval, &market, start_ms, iv_secs, max_requests));
        match res {
            Ok(candles) if !candles.is_empty() => {
                ...
                println!("  ✅ {:12} {} bar ({} → {})", sym, saved, first, last);
                ok += 1;
                total_rows += saved;
            }
    ...
    println!("\n→ {} / {} sembol indirildi · toplam {} bar yazıldı.", ok, symbols.len(), total_rows);
}
