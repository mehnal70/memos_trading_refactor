...
// Coinbase için ExchangeConnector trait implementasyonu (websocket + REST)
...
impl ExchangeConnector for CoinbaseConnector {
    fn exchange(&self) -> Exchange {
        Exchange::Binance // TODO: Exchange::Coinbase eklendiğinde değiştir
    }

    async fn start_price_stream(&mut self, _symbol: &str, _on_price: Box<dyn Fn(f64) + Send + Sync>) {
        // Coinbase websocket ile fiyat stream başlat
    }
