...
mod tests {
    ...
    use crate::robot::{InMemoryStateManager, DummyTradeExecutor, StdoutLogger};
    use crate::types::Signal;

    #[test]
    fn test_executor_with_state_and_logging() {
        let mut state = InMemoryStateManager::new();
        state.set_symbols(vec!["BTCUSDT".to_string(), "ETHUSDT".to_string()]).unwrap();
        ...
        let trades = robot.execute_basket(Signal::Buy, 0.1);
        logger.log_info("test_executor", &format!("{} trade executed", trades.len()));
        for t in &trades {
            match t {
                Ok(trade) => logger.log_info("trade", &format!("Trade: {} {}", trade.symbol, trade.amount)),
                Err(e) => logger.log_error("trade", &format!("Error: {:?}", e)),
            }
        }
        assert!(trades.len() == 2);
    }
}
// robot/executor.rs - Robotik Trade Executor (Scheduler, basket, otomasyon)

use crate::robot::interfaces::TradeExecutor;
use crate::robot::state_manager::StateManager;
