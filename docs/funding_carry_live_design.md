...
- **Mod-geçişi orphan (operasyonel — `.launch.conf` modunu değiştirirken):** Adanmış modlar arası geçişte (ör. BLEND→CARRY veya XS→CARRY) önceki modun açık pozisyonları YETİM kalır — her kitap yalnız kendi strat-etiketli pozisyonunu (FUNDING_CARRY/BLEND_FACTOR/...) yönetir; yeni kitap yabancı-etiketliyi ne devralır ne kapatır, tek-pozisyon/sembol invariantı da yeni kitabı o sembolde bloklar → dengeli kitap kurulamaz (ilk smoke 4 BLEND_FACTOR artığıyla böyle kirlendi). **ÇÖZÜM:** mod değiştirmeden önce flatten — `./memos down && ./memos reset -y && ./memos up` (`reset --state` pozisyonları boşaltır + equity'yi başlangıca sıfırlar; öğrenilmiş durum/mumlar korunur, `.dump` yedeği alınır). İleride opsiyonel kod iyileştirmesi: boot'ta sepet sembolündeki yabancı-etiketli adanmış-kitap pozisyonunu kapatan reconcile adımı.

## 7. Doğrulama planı (kod sonrası)
1. ✅ Paper smoke (2026-06-11): temiz boot'ta dengeli carry kitabı kuruldu (3 SHORT yüksek-funding BTC/DOGE/LINK + 3 LONG düşük-funding SOL/BCH/DOT, hepsi ~$1000 eşit-ağırlık, FUNDING_CARRY), invariant tuttu (sepet-dışı SUPERTREND yan yana). ÖNKOŞUL: mod-geçişinde flatten (bkz. §6 orphan maddesi).
