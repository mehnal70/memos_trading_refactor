...
INTEG="reports/integration_${TS}.env"
{
    ...
    echo "EDGE_SEED_REPORT=${EDGE_JSON}"
    echo "# İndikatör/osilatör paramları: ParameterStore yalnız wf_robust şampiyonları yükler."
    echo "# (per-sembol; şampiyon stratejisi canlı seçilen stratejiyle eşleşince uygulanır)."
    echo "PARAM_SWEEP_REPORT=${PARAM_JSON}"
    echo "# PARAM_SWEEP_REQUIRE_WF=0   # WF kapısını gevşetmek için (fluke riski; default açık)"
